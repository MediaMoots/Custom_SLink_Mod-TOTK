...
HOOK_DEFINE_REPLACE(searchAssetCallTableByName) {
    static bool Callback(uintptr_t self, xlink2::Locator * locator, char * searchKeyName) {
            ...
            if (iVar8 != 0) {
                ...
                if (0 < iVar8 && lVar6 != 0) {
                    ...
                    do {
                        ...
                        iVar1 = iVar1 >> 1;
                        searchResult = (xlink2::ResAssetCallTable * )
                            (lVar6 + (ulong) * (ushort * )(lVar5 + (long) iVar1 * 2) * 0x30);
        ...
        FOUND_ITEM:
            locator -> mResAssetCallTable = searchResult;

        if (locator && locator -> mResAssetCallTable) {
            // PRINT("Valid Locator Found")
